...
CREATE UNIQUE INDEX "dagets_one_active_instant" ON "dagets" USING btree ("creator_user_id") WHERE status = 'active' AND daget_type::text != 'raffle';--> statement-breakpoint
CREATE UNIQUE INDEX "dagets_one_active_raffle" ON "dagets" USING btree ("creator_user_id") WHERE status::text IN ('active', 'drawing') AND daget_type::text = 'raffle';--> statement-breakpoint
ALTER TABLE "dagets" ADD CONSTRAINT "raffle_ends_at_required" CHECK (daget_type::text != 'raffle' OR raffle_ends_at IS NOT NULL);--> statement-breakpoint
ALTER TABLE "dagets" ADD CONSTRAINT "raffle_ends_at_forbidden" CHECK (daget_type::text = 'raffle' OR raffle_ends_at IS NULL);--> statement-breakpoint
ALTER TABLE "dagets" ADD CONSTRAINT "drand_raffle_only" CHECK (daget_type::text = 'raffle' OR (drand_round IS NULL AND drand_randomness IS NULL));--> statement-breakpoint
ALTER TABLE "dagets" ADD CONSTRAINT "claimed_count_range" CHECK (
    claimed_count >= 0
    AND ((daget_type::text != 'raffle' AND claimed_count <= total_winners) OR daget_type::text = 'raffle')
  );--> statement-breakpoint
ALTER TABLE "dagets" ADD CONSTRAINT "random_mode_check" CHECK (
    (daget_type::text = 'fixed' AND random_min_bps IS NULL AND random_max_bps IS NULL)
    OR
    (daget_type::text = 'random'
    ...
    OR
    (daget_type::text = 'raffle' AND random_min_bps IS NULL AND random_max_bps IS NULL)
  );--> statement-breakpoint
